...
export async function GET(request: Request) {
  try {
    ...
    // 2️⃣ Strip "Bearer " if it exists
    const tokenToSend = authHeader.startsWith('Bearer ')
      ? authHeader.slice(7)
      : authHeader;
    ...
    const res = await fetch(`${backendUrl}/api/assignments/all`, {
      headers: {
        'Authorization': tokenToSend  // Forward the auth header
      }
    });
